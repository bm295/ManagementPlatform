...
public final class ManagementPlatformHttpAdapter {
    ...
    private final InMemoryCheckoutRepository checkoutRepository;
    private final InMemoryDeadLetterRepository deadLetterRepository;
    private final CheckoutUseCase checkoutUseCase;

    public ManagementPlatformHttpAdapter(InMemoryOrderRepository orderRepository,
    ...
    public HttpServer createServer(int port) throws IOException {
        ...
        server.createContext("/api/checkouts", this::handleCheckouts);
        server.createContext("/api/dead-letters", this::handleDeadLetters);
        server.setExecutor(null);
        return server;
    }

    private void handleHealth(HttpExchange exchange) throws IOException { if (!"GET".equals(exchange.getRequestMethod())) { sendMethodNotAllowed(exchange, "GET"); return; } sendJson(exchange, 200, "{\"status\":\"ok\"}"); }
    private void handleRoot(HttpExchange exchange) throws IOException { if (!"GET".equals(exchange.getRequestMethod())) { sendMethodNotAllowed(exchange, "GET"); return; } if (!"/".equals(exchange.getRequestURI().getPath())) { sendProblem(exchange, 404, "Not Found", "Route was not found."); return; } exchange.getResponseHeaders().set("Content-Type", "text/html; charset=utf-8"); byte[] body = "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>Management Platform</title></head><body><main><h1>Management Platform API</h1><p>The service is running.</p></main></body></html>".getBytes(StandardCharsets.UTF_8); exchange.sendResponseHeaders(200, body.length); try (OutputStream responseBody = exchange.getResponseBody()) { responseBody.write(body); } }
